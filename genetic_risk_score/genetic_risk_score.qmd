---
title: "Genetic Risk Scores for Type 2 Diabetes Using Python"
format: gfm
---
...
- $\log(\text{Odds Ratio})$ is the natural logarithm of the odds ratio associated with each SNP's risk allele.
- $\text{Allele Count}$ is the number of risk alleles ($0$, $1$, or $2$) present in an individual's genotype for each SNP.
...
The [genotypes.tsv](https://github.com/ahmedmoustafa/datasets/blob/main/genetic_risk_score/genotypes.tsv) table of 1000 individuals, where each row represents an individual, and each column represents a SNP.
